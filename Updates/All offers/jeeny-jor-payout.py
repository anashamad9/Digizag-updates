...
AFFILIATE_SHEET = "Jeeny Jo"               # coupons sheet for this offer
REPORT_PREFIX   = "jor-digizag-report-"
REPORT_EXT      = ".csv"
...
def extract_date(filename):
    """From 'jor-digizag-report-YYYY-MM-DD.csv' return a datetime for sorting."""
    m = re.search(r'jor-digizag-report-(\d{4}-\d{2}-\d{2})', filename)
    if m:
        try:
...
jor_files = [f for f in os.listdir(input_dir) if f.startswith(REPORT_PREFIX) and f.endswith(REPORT_EXT)]
if not jor_files:
    raise FileNotFoundError(f"No files starting with '{REPORT_PREFIX}' found in the input directory.")

latest_file = max(jor_files, key=extract_date)
...
# Window (keep last N days inclusive of end_date)
...
df_expanded['revenue'] = 1.0      # JOR revenue/unit

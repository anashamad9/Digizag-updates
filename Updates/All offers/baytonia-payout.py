...
REPORT_PREFIX    = "DigiZag__Baytonia_ Orders Tracking Report_DigiZag_Table"  # dynamic CSV name start
...
# Dynamically pick the changing report CSV
...
# =======================
# sale_amount in USD
df_filtered['sale_amount'] = df_filtered['Total'] * USD_PER_SAR

# revenue varies by customer type: New -> 5%, else 2%
df_filtered['revenue'] = df_filtered.apply(
    lambda row: row['sale_amount'] * 0.05 if str(row.get('Cutomer Type', '')).strip() == 'New'
                else row['sale_amount'] * 0.02,
    axis=1
)

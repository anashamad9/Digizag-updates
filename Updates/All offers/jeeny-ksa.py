...
# Find the latest ksa-digizag-report- file in the input directory
ksa_digizag_files = [f for f in os.listdir(input_dir) if f.startswith('ksa-digizag-report-') and f.endswith('.csv')]
if not ksa_digizag_files:
    raise FileNotFoundError("No files starting with 'ksa-digizag-report-' found in the input directory.")

# Extract and sort by date using regex
def extract_date(filename):
    match = re.search(r'ksa-digizag-report-(\d{4}-\d{2}-\d{2})', filename)
    if match:
        return datetime.strptime(match.group(1), '%Y-%m-%d')
    return datetime.min  # Default to min date if no match

latest_file = max(ksa_digizag_files, key=extract_date)

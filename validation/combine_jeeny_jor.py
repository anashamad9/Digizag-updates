"""
Combine all Jeeny JOR CSV files into a single CSV.

The script targets `validation/jeeny-jor` by default and writes the
combined data to `jeeny_jor_monthly.csv` in the same folder.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Combine Jeeny JOR CSV files into one file.",
    )
    default_source = Path(__file__).with_name("jeeny-jor")
    parser.add_argument(
        "--source",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output CSV path (default: <source>/jeeny_jor_monthly.csv)",
    )
...
def main() -> None:
    ...
    output_path = (
        ...
        if args.output
        else source_dir / "jeeny_jor_monthly.csv"
    )

...
# CONFIG (REEF)
# =======================
OFFER_ID = 1341  # <<< IMPORTANT: Only 1341
...
SOURCE_RESOURCE = os.getenv("REEF_SOURCE", SOURCE_RESOURCE_DEFAULT)
AFFILIATE_XLSX_PRIMARY = "Offers Coupons.xlsx"
AFFILIATE_SHEET = "Reef Perfumes"  # change if your tab name differs
OUTPUT_CSV = "reef.csv"
...
def fetch_json_resource(resource: str, timeout: int = 60):
    ...
    if not resource.lower().startswith(('http://', 'https://')):
        raise ValueError(
            "REEF_SOURCE must be an HTTP(S) endpoint or an existing file path. "
            f"Got: {resource}"
        )
...
def load_source(resource: str) -> pd.DataFrame:
    ...
    if not os.path.exists(resource):
        raise FileNotFoundError(f"Reef data source not found: {resource}")

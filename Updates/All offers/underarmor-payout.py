...
REPORT_CSV      = "UA - Digizag Data Studio Report by GMG_Untitled Page_Table.csv"
AFFILIATE_XLSX  = "Offers Coupons.xlsx"
AFFILIATE_SHEET = "Under Armour"   # change if your tab name differs (e.g., "UA", "UnderArmour")
...
def normalize_coupon(x: str) -> str:
    """Uppercase, trim, take first token if multiple codes are separated by ; , or whitespace."""
    if pd.isna(x):
        return ""
    s = str(x).strip().upper()
...
# Convert 'Order Date' and filter range (inclusive of end_date per your draft)
...
# =======================
# Sale amount (AED -> USD)
df['sale_amount'] = pd.to_numeric(df['Net Sales (in AED)'], errors='coerce').fillna(0.0) / 3.67
